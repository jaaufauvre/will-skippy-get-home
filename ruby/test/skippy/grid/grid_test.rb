...
describe Grid::Grid do
  ...
  it 'should place item at the given location' do
    # GIVEN
    grid_item = Grid::GridItem.new
    grid = Grid::Grid.new(10)

    # WHEN
    grid.place_item(grid_item, Grid::Point.new(3, 4))

    # THEN
    _(grid.get_item_location(grid_item)).must_equal(Grid::Point.new(3, 4))
  end

  it 'should raise ItemExistsError when placing an item already on the grid' do
    # GIVEN
    grid_item = Grid::GridItem.new
    grid = Grid::Grid.new(2)
    grid.place_item(grid_item, Grid::Point.new(0, 0))

    # WHEN & THEN
    error = assert_raises Grid::Grid::ItemExistsError do
      grid.place_item(grid_item, Grid::Point.new(1, 1))
    end
  ...
  it 'should raise OutOfBoundsError when an item is placed outside the grid' do
    # GIVEN
    grid_item = Grid::GridItem.new
    grid = Grid::Grid.new(1)

    # WHEN & THEN
    error = assert_raises Grid::Grid::OutOfBoundsError do
      grid.place_item(grid_item, Grid::Point.new(1, 0))
    end
    _(error.location).must_equal(Grid::Point.new(1, 0))
  ...
  it 'should move item in the given direction' do
    ...
    # WHEN & THEN
    _(grid.move_item(grid_item, Direction::NORTH)).must_equal(Grid::Point.new(3, 5))
    _(grid.get_item_location(grid_item)).must_equal(Grid::Point.new(3, 5))
    _(grid.move_item(grid_item, Direction::SOUTH)).must_equal(Grid::Point.new(3, 4))
    _(grid.get_item_location(grid_item)).must_equal(Grid::Point.new(3, 4))
    _(grid.move_item(grid_item, Direction::EAST)).must_equal(Grid::Point.new(4, 4))
    _(grid.get_item_location(grid_item)).must_equal(Grid::Point.new(4, 4))
    _(grid.move_item(grid_item, Direction::WEST)).must_equal(Grid::Point.new(3, 4))
    _(grid.get_item_location(grid_item)).must_equal(Grid::Point.new(3, 4))
  end
  ...
  it 'should raise OutOfBoundsError when an item is moved outside the grid' do
    ...
    _(error.location).must_equal(Grid::Point.new(0, -1))
  ...
  it 'should raise ItemNotFoundError when moving an item not on the grid' do
    ...
    # WHEN & THEN
    error = assert_raises Grid::Grid::ItemNotFoundError do
      grid.move_item(grid_item, Direction::SOUTH)
    end
